...
class _WorldRenderPanelState extends State<WorldRenderPanel> with SingleTickerProviderStateMixin {
    ...
    return LayoutBuilder(builder: (ctx, constraints) {
      ...
      return Stack(children: [
        ...
        ...widget.cities.entries.map((e) {
          ...
          return Positioned(
            left: pos.dx - 56, top: pos.dy - 18, width: 112,
        ...
        if (widget.narration.isNotEmpty)
          Positioned(bottom: 6, left: 8, right: 8, child: _narratorBar()),
      ]);
    });
  }
  ...
  List<Widget> _agentDots(double tileW, double tileH) {
    ...
    for (final e in byCity.entries) {
      ...
      final ids = e.value;
      final startX = center.dx - (ids.length * 12.0) / 2;
      for (int i = 0; i < ids.length; i++) {
        final agentColor = Noir.agent(ids[i]);
        final active = widget.events.any((ev) => ev.agent == ids[i]);
        out.add(Positioned(
          left: startX + i * 12, top: center.dy + 14,
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: const Color(0xEE0D1117), borderRadius: BorderRadius.circular(5),
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Row(mainAxisSize: MainAxisSize.min, children: [
          ...
          const SizedBox(width: 6),
          Text('${tiles}t', style: TextStyle(color: color.withOpacity(0.6), fontSize: 7, fontFamily: 'JetBrains Mono')),
        ]),
      ]),
    );
  }
