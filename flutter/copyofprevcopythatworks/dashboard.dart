...
class _WorldDashboardState extends State<WorldDashboard> {
      ...
      try {
        switch (pkt.pid) {
          case S2C.handshakeAck:
            _clientId = pkt.data['client_id']?.toString();
            _tick = (pkt.data['current_tick'] is num)
                ? (pkt.data['current_tick'] as num).toInt() : _tick;
            _activeModules = List<String>.from(pkt.data['active_modules'] ?? []);
            _phase = pkt.data['state']?.toString() ?? 'SPECTATE';
            _connected = true;

          case S2C.worldSnapshot:
            _tick = (pkt.data['tick'] is num)
                ? (pkt.data['tick'] as num).toInt() : _tick;
            _parseCities(pkt.data['world']);
            if (pkt.data['economy'] is Map<String, dynamic>) {
          ...
          case S2C.roundResult:
            ...
            if (pkt.data['events'] is List) {
              _events = (pkt.data['events'] as List)
          ...
          case S2C.cityUpdate:
            final cityId = pkt.data['city_id']?.toString();
            final cityData = pkt.data['city'];
            if (cityId != null && cityData is Map<String, dynamic>) {
        ...
        print('[DASHBOARD] Error parsing packet ${pkt.pid}: $e');
      }
    });
  }
  ...
  void _parseCities(dynamic worldData) {
    if (worldData is! Map<String, dynamic>) return;
    final citiesRaw = worldData['cities'];
    if (citiesRaw is! Map<String, dynamic>) return;
